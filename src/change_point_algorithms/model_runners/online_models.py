...
ResultType: TypeAlias = tuple[DetectionAlgorithm | DetectionAlgorithmV2, RegionType, RegionType]

def run_online_models(
        time: np.ndarray, data: np.ndarray,
        models: Iterable[DetectionAlgorithm]) -> list[ResultType]:
    ...
    for model in models:
        ...
        match model.type:
            case ModelType.BOCPD:
                shocks, non_shocks = bocpd.get_bocpd_from_generator(
            ...
            case ModelType.EM:
                ...
                shocks, non_shocks = get_em_model_from_generator(
                    time, hp.normal_data_size, hp.abnormal_data_size,
                    data, hp.normal_mean, hp.abnormal_mean,
                    hp.normal_var, hp.abnormal_var, hp.pi, hp.epochs,
                    with_progress=model.with_progress
                )
                # rng = np.random.default_rng()
                # safe = rng.normal(
                ...
                # shocks, non_shocks = get_em_from_generator(
            ...
            case ModelType.CUSUM:
                ...
                shocks, non_shocks = cusum.get_cusum_from_generator(
                    ...
                    version=hp.version, with_progress=model.with_progress
                )
            case ModelType.GREY_MODEL:
                hp: Hyperparameters.GreyHyperparams = model.hyperparameters
                shocks, non_shocks = get_grey_from_generator(
...
def run_online_models_v2(
        time: np.ndarray, data: np.ndarray,
        models: Iterable[DetectionAlgorithmV2], with_progress: bool = False) -> list[ResultType]:
    """ Run each model in the models list on given data and return results.
    ...
    for model in models:
        match model.type:
            ...
            case ModelType.CUSUM:
                ...
                shocks, non_shocks = cusum.get_cusum_from_generator(
                    time, data, hp.mean, hp.std_dev, hp.h, hp.alpha,
                    version=hp.version, with_progress=with_progress
                )
            case ModelType.GREY_MODEL:
                hp: Hyperparameters.GreyHyperparams = model.hyperparameters
                shocks, non_shocks = get_grey_from_generator(
...
#             # sub_df = df.filter(name=model_name, is_change_point=1).select('time')
#             sub_arr = df.filter(name=model_name, is_change_point=1).select('time').to_numpy()
#             for (start, stop) in sliding_window_view(sub_arr, 2):
#                 pair = (start, stop)
