...
from change_point_algorithms.fig_funcs.signal_plots import plot_signal_for_display, plot_signal_fft, plot_signal_power_spectrum, \
    plot_signal_for_flowchart
from change_point_algorithms.fig_funcs.spectrogram_plots import get_spectrogram, plot_spectrogram_for_display
...
def make_signal_plots(time, data, save_root=None):
    ...
    # plt.savefig(Path(save_dir, 'flowchart_time_fig.pdf'))
...
# def make_signal_plots_for_paper(time, data, save_root=None):
#     """ Make plots to be used in paper."""
#     save_dir = Path(save_path(save_root), SIGNAL_PLOT_SUB_DIR)
#     my_dpi = 350
#     fig_size = (6.5, 2)
#     # Plots for whole signal
#     fig = plot_signal_for_display(time, data, ms=True, fig_size=fig_size)
#     plt.savefig(Path(save_dir, 'signal_paper_fig.png'), dpi=my_dpi)
#     plt.close(fig)
