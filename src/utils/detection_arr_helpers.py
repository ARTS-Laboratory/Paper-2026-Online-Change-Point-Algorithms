...
def intervals_to_dense_arr(time, shock_intervals, non_shock_intervals) -> np.ndarray:
...
def make_time_index_cache(time_arr: np.typing.ArrayLike) -> dict[float, int]:
    """ """
...
def convert_intervals_to_time(time, intervals):
    match time:
        ...
        case _:
            raise TypeError(f"Expected time to be list or np.ndarray, got {type(time)}")
...
def convert_intervals_to_time_v2(time, intervals, cache_dict=None):
    if cache_dict is not None:
        return convert_intervals_to_time_memoized(time, intervals, cache_dict)
    else:
